...
- `index.html` — **wird aus den `sections/*.html`-Fragmenten zusammengesetzt** (Kopf + Fragmente in fester Reihenfolge, siehe Skript-Muster in der Git-Historie). Nicht direkt in `index.html` editieren — Änderungen gehen beim nächsten Neuaufbau verloren. Immer das jeweilige `sections/*.html` ändern und `index.html` neu bauen. (Ist schon einmal passiert: ein Fix landete nur als Inline-`<script>` in `index.html`, nicht in einer `sections/`-Datei, und wäre bei einem Rebuild verloren gegangen.)
